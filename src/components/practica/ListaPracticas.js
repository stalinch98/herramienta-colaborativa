    ...
    return (
        <div>
            <div ref={(el) => (componentToPrint.current = el)}
                 style={{margin: '20px', padding: '20px', fontFamily: 'Times New Roman', fontSize: '22px'}}>
...
function ListadoPracticas({idAsignatura, tipo}) {
    ...
    const [isModalDescargar, setIsModalDescargar] = useState(false);
    const [dataModalDescargar, setDataModalDescargar] = useState('');
    ...
    const downloadPDF = async (item, tipodescarga) => {

        try {
            let practica = await clienteAxios.get(`${process.env.REACT_APP_BACKEND_URL}/api/practica/pdf/${item._id}/${tipodescarga}`);
            setDataModalDescargar(practica.data.data);
            setIsModalDescargar(true);
        } catch (error) {
            mostrarMsg("Descargar practica", "error");
        }
    ...
    return (
        <>
            ...
            <Modal
                ...
                width={900}
            >
                <DownloadPDF data={dataModalDescargar}/>
            </Modal>
        </>
    );
}
